...
instance Entity Sequence where
  name = sequenceName
  render s = text "CREATE SEQUENCE" <+>
             renderRegularIdentifier (sequenceName s) $+$
    ...
    where
      ...
      startWith' = maybe empty (\ x -> text "START WITH" <+> integer x)
                   (startWith s)
      incrementBy' = maybe empty (\ x -> text "INCREMENT BY" <+> integer x)
                     (incrementBy s)
      minValue' = maybe empty renderMinValue (minValue s)
      maxValue' = maybe empty renderMaxValue (maxValue s)
      cycle' = maybe empty (\ x -> text (if x then "CYCLE" else "NO CYCLE"))
               (cycle s)
...
numericBounds (Just BigInt) =
  Just (- 9223372036854775808, 9223372036854775807)
...
{-
 Bug.  If used for min value, value can't be upper bound
 (e.g. min value 255 for TinyInt)
-}
...
arbitraryValue (Just BigInt) =
  boundedMaybeInt (- 9223372036854775808, 9223372036854775807)
arbitraryValue _ = oneof [liftM Just arbitrary, return Nothing]

arbitraryCacheValue :: Gen (Maybe Integer)
arbitraryCacheValue = frequency
                      [(50, liftM Just $ choose (1, 500)), (50, return Nothing)]
...
-}
validIncrementBy :: Maybe NumericType ->
                    Maybe Integer ->
...
validIncrementBy' :: NumericType ->
                     Maybe Integer ->
                     Maybe Integer -> Maybe Integer -> Bool
validIncrementBy' Decimal min' max' incr' =
  validIncrementBy' Numeric min' max' incr'
validIncrementBy' Numeric min' max' incr' =
  maybe
  True
  (\ absDiff -> maybe True (\ x -> abs x <= abs absDiff) incr')
  (liftM2 (-) max' min')
validIncrementBy' x min' max' incr' =
  maybe True (\ incr -> abs incr <= diff) incr'
  where
...
instance Arbitrary Sequence where
  arbitrary = do
    nm <- arbitrary
    dataType <- arbitrary
    minV <- arbitraryValue dataType `suchThat`
            validMinimum dataType
    maxV <- arbitraryValue dataType `suchThat`
            \ x -> greaterThanMin minV x && validMaximum dataType x
    start <- arbitraryValue dataType `suchThat`
             \ x -> greaterThanMin minV x && lessThanMax maxV x
    increment <- arbitraryValue dataType `suchThat`
                 validIncrementBy dataType minV maxV
    cyc <- arbitrary
