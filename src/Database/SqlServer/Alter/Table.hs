...
selectValidColumnForAdd t =
  arbitrary `suchThat`
  (\ x -> columnConstraintsSatisfied (x : unpack (columns t)))
...
renderAddColumn t c =
  text "ALTER TABLE" <+> renderName t <+> text "ADD" <+> renderColumn c $+$
  text "GO\n"

renderDropColumn :: Table -> Column -> Doc
renderDropColumn t c =
  text "ALTER TABLE" <+> renderName t <+>
  text "DROP" <+> renderRegularIdentifier (columnName c) $+$
  text "GO\n"
